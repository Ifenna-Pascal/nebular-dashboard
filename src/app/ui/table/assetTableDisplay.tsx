...
const AssetTableDisplay:FC<IAssetTable> = ({styles, header, colmumHeads, data}) => {
  return (
    <div className={`border border-[#302E2E] p-6  rounded-[20px] min-h-[200px] ${styles}`}>
        <h1 className='text-white font-montserrat font-semibold text-[18px] border-b border-[#302E2E] pb-5'>{header}</h1>
        <div className="overflow-x-auto">
         <table className="min-w-full">
                ...
                <tr className="h-[65px]">
                    {
                        colmumHeads.map((title, index) => (
                            ...
                            <div className={`flex ${index === 0 ? 'justify-start' : 'justify-center'} items-center`}>
                                <span className="text-light-60 text-center font-montserrat text-[14px] font-semibold">{title}</span>
                                {
                                    title !== '' && <Image
                                    src={assets.sortIcon}
            ...
            <tbody>
                    {
                        data.map((info, index) => (
                        <tr className="h-[65px] hover:bg-light-10 pl-8 cursor-pointer mb-4 hover:rounded-[18px]" key={index}>
                            <td
                                scope="col"
